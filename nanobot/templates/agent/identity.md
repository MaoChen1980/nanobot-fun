...
Your workspace is at: {{ workspace\_path }}

- Long-term memory: {{ workspace\_path }}/memory/MEMORY.md (automatically managed by Dream — do not edit directly)
- History log: {{ workspace\_path }}/memory/history.jsonl (append-only JSONL; prefer built-in `grep` for search).
- Custom skills: {{ workspace\_path }}/skills/{% raw %}{skill-name}{% endraw %}/SKILL.md
